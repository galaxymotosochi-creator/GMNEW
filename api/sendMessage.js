...
// Вызывается с galaxymoto.ru (GitHub Pages) через кросс-доменный fetch
module.exports = async (req, res) => {
  // CORS — разрешаем только наш домен
  const origin = req.headers.origin || '';
  const ALLOWED = [
    'https://www.galaxymoto.ru',
    'https://galaxymoto.ru',
    'https://gmnew-pied.vercel.app',
    'https://galaxymotosochi-creator.github.io'
  ];
  if (ALLOWED.includes(origin)) {
    res.setHeader('Access-Control-Allow-Origin', origin);
  } else {
    // Если origin нет (curl, Postman) — тоже разрешаем
